...
### 冲突分析
v0.7.0的压缩功能与搜索功能存在潜在冲突：
- **压缩数据**需要解压才能搜索，影响搜索性能
- **目标**：搜索响应时间 < 100ms（P95）
- **挑战**：解压操作可能增加50-200ms延迟

### 解决策略（3种）

#### 策略1：冷热数据分层策略
- **热数据**：不压缩，保证搜索速度
- **温数据**：轻量压缩（LZ4 fast），平衡性能与空间
- **冷数据**：深度压缩（Zstd high ratio），最大化空间节省
- **适用场景**：访问模式稳定，搜索频率中等
...
#### 策略3：索引独立存储
- **搜索索引**不受压缩影响，独立存储（额外5%存储开销）
- **原始文件**可压缩，但索引保持可读状态
- **优势**：搜索无需解压，P95 < 100ms SLA可轻松满足
- **适用场景**：搜索密集型应用，文档管理系统

### 混合策略（默认推荐）
结合三种策略的优点，针对不同数据采用不同策略：
```rust
match file_type {
    // 搜索频繁的文件
    TextDocument | CodeFile | LogFile =>
        StorageTier::Hot + 不压缩 + 独立索引,

    // 偶尔搜索的文件
    Archive | Backup =>
        StorageTier::Warm + 轻量压缩 + 索引独立,

    // 无需搜索的文件
    MediaFile | Database | Binary =>
        StorageTier::Cold + 深度压缩,
}
```

### 配置实现
```yaml
# storage_v2/config.yaml
compression:
  strategy: "hybrid"  # hybrid | hot_only | cold_only | independent_index
  search_aware: true
  hot_tier_compression: false
  warm_tier_compression: "lz4"
  cold_tier_compression: "zstd"

search:
  index_independent: true  # 索引独立存储
  index_size_limit: "10%" # 索引大小限制为原文件10%
  decompress_cache: true   # 缓存解压结果
```

### 性能目标
| 策略 | 搜索速度 | 存储节省 | 额外开销 | SLA达成率 |
|------|---------|---------|---------|----------|
| 冷热分层 | 中等 | 70% | 0% | 95% |
| 搜索时解压 | 低 | 70% | 0% | 80% |
| 索引独立 | **高** | 65% | **5%索引** | **99%** |
...
- **默认配置**：混合策略，适合大多数场景
- **性能调优**：根据实际使用模式选择策略
- **监控指标**：搜索延迟分布、压缩比、索引大小
- **告警阈值**：搜索P95 > 100ms 自动告警
