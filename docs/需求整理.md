...
## 变更记录
- 2025-10-17 22:00:00 +0800：完成用户认证与授权系统（FR-3.4），新增 `src/auth/` 模块（models/jwt/password/storage/mod），实现基于JWT的完整认证系统；支持Argon2密码哈希、用户角色管理、Sled数据库存储；新增5个认证API端点（register/login/refresh/me/password）；52个单元测试全部通过，覆盖率82-94%；新增完整用户文档（docs/认证功能文档.md）；通过环境变量ENABLE_AUTH可选启用。
