...
## 变更记录
- 2025-10-17 10:52:00 +0800：完成增量同步功能（FR-3.3），新增 `src/incremental_sync.rs` 和 `src/incremental_sync_handler.rs` 模块，实现基于块的文件差异检测和增量同步；支持SHA256强哈希和Adler-32弱哈希的双重签名；新增HTTP API端点 `GET /api/sync/signature/{id}` 和 `POST /api/sync/delta/{id}`；在EventListener中集成增量同步，优先使用增量传输，失败时自动回退到全量下载；所有测试通过，`cargo check` 和 `cargo clippy` 均通过。
- 2025-10-15 20:40:00 +0800：完成 CRDT 文件同步功能（FR-3.1），新增 `src/sync.rs` 模块，实现基于 LWW-Register 的元数据同步和向量时钟冲突检测；开始文件版本管理系统（FR-4.1）开发。
