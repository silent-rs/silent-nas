...
mod tests {
    ...
    use crate::storage_v2::{IncrementalStorage, IncrementalConfig};
    ...
    async fn test_incremental_storage_efficiency() {
        ...
        let mut incremental = IncrementalStorage::new(
            storage,
            config,
        ...
        println!("\n=== 增量存储效率测试 ===");
        println!("新文件保存: {} 字节数据，产生 {} 个块", 
            data1.len(), delta1.chunks.len());
        ...
        println!("增量更新: {} 字节数据，产生 {} 个块（vs 完整重新存储）", 
            data2.len(), delta2.chunks.len());
    ...
    fn test_version_chain_efficiency() {
        ...
        let chain_total = base_size + (base_size as f32 * change_ratio * (version_count - 1) as f32) as usize;
